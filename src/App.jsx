// src/App.jsx — واجهة الزبون لبيتزا خانم
// ✅ البيانات من MongoDB عبر /api/pizzas
// ✅ الصور من Cloudinary (imageUrl / flavorImageUrl)
// ✅ Fallback للبيانات الافتراضية لو قاعدة البيانات فارغة
...
const DEFAULT_MENU = [
  { id:"4seasons",      label:"الفصول الأربعة",  details:"جبنة القشقوان مع الماشروم والزيتون الأسود والفليفلة الخضراء.", comingSoon:false, imageUrl:"", flavorImageUrl:"" },
  { id:"margarita",    label:"مارغريتا",          details:"جبنة القشقوان مع الصلصة الحمراء.", comingSoon:false, imageUrl:"", flavorImageUrl:"" },
  { id:"hawaii",       label:"هاواي",             details:"جبنة القشقوان مع الموزريلا وشرائح الأناناس.", comingSoon:false, imageUrl:"", flavorImageUrl:"" },
  { id:"teamscheese",  label:"التيمات تشيز",      details:"جبنة القشقوان مع موزريلا وكرات الطماطم والريحان.", comingSoon:false, imageUrl:"", flavorImageUrl:"" },
  { id:"supersupreme", label:"سوبر سوبريم",       details:"البيروني مع جبنة القشقوان والماشروم والفلفل الأخضر والزيتون الأسود.", comingSoon:false, imageUrl:"", flavorImageUrl:"" },
  { id:"chickenbbq",   label:"تشيكن باربيكيو",   details:"شرائح الدجاج بصوص الباربيكيو وجبنة القشقوان مع البصل.", comingSoon:false, imageUrl:"", flavorImageUrl:"" },
  { id:"peperoni",     label:"ببروني",            details:"جبنة القشقوان مع شرائح البيروني لحم البقر والثوم والكزبرة.", comingSoon:false, imageUrl:"", flavorImageUrl:"" },
  { id:"salami",       label:"سلامي",             details:"جبنة القشقوان مع شرائح لحم البقر.", comingSoon:false, imageUrl:"", flavorImageUrl:"" },
  { id:"hotdog",       label:"هوت دوغ",           details:"جبنة القشقوان مع حبات الهوت دوغ المدخن.", comingSoon:false, imageUrl:"", flavorImageUrl:"" },
  { id:"smokedchicken",label:"دجاج مدخن",         details:"جبنة القشقوان مع شرائح دجاج الحبش المدخن.", comingSoon:false, imageUrl:"", flavorImageUrl:"" },
  { id:"fajita",       label:"فاهيتا",            details:"جبنة القشقوان مع دجاج الفاهيتا والماشروم والفلفل الأخضر والذرة.", comingSoon:false, imageUrl:"", flavorImageUrl:"" },
  { id:"cs1", label:"بيتزا الكريمة", details:"", comingSoon:true, imageUrl:"", flavorImageUrl:"" },
  { id:"cs2", label:"الثلاثي",        details:"", comingSoon:true, imageUrl:"", flavorImageUrl:"" },
  { id:"cs3", label:"بيتزا البحر",   details:"", comingSoon:true, imageUrl:"", flavorImageUrl:"" },
  { id:"cs4", label:"المكسيكية",      details:"", comingSoon:true, imageUrl:"", flavorImageUrl:"" },
  { id:"cs5", label:"بيتزا الخضار",  details:"", comingSoon:true, imageUrl:"", flavorImageUrl:"" },
];
const SIZES_REGULAR = [
...
/* ══ PIZZA IMAGE ══════════════════════════════════════════════════════════════
   يعرض صورة من Cloudinary (imageUrl) أو placeholder جميل
════════════════════════════════════════════════════════════════════════════ */
      ...
      <div className="featured-scroll" ref={scrollRef} onScroll={onScroll}>
        {featured.map((fp, i) => (
          <div key={fp.id} className={`featured-card${i===active?" acard":""}`} onClick={()=>onCardClick(fp)}>
            <PizzaImg imageUrl={fp.imageUrl} label={fp.label} style={{width:"100%",height:160,borderRadius:0}}/>
...
export default function PizzaKhanum() {
  ...
  /* ── بيانات الإعدادات (من localStorage) ── */
  ...
  /* ── FLAVORS: مشتقة من قائمة الـ Menu (تُحدَّث عند تحميل البيانات) ── */
  const flavors = useMemo(() =>
    pizzasMenu.map(p => ({ id:p.id, label:p.label, comingSoon:p.comingSoon, flavorImageUrl:p.flavorImageUrl||"" })),
    [pizzasMenu]
  );
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [menuRes, featRes] = await Promise.all([
        ...
        if (menuRes.ok) {
          ...
          setPizzasMenu(items.length > 0 ? items : DEFAULT_MENU);
        } else {
          setPizzasMenu(DEFAULT_MENU);
        }

        if (featRes.ok) {
          const json = await featRes.json();
          const items = json.data || [];
          setFeatured(items.length > 0 ? items : DEFAULT_FEATURED);
        } else {
        ...
        {flavors.map(f => {
          ...
          return (
            <div key={f.id} className={`flavor-btn${f.comingSoon?" cs":cnt>0?" hfl":""}`} onClick={()=>{if(f.comingSoon)return;onPick(f.id);}}>
              <PizzaImg imageUrl={f.flavorImageUrl} label="" style={{width:"100%",height:72,borderRadius:0}}/>
  ...
  /* ════ LOADING ════ */
  if (loading) return (
    <div dir="rtl" style={{fontFamily:"'Noto Kufi Arabic',sans-serif",background:"#0f0f0f",minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16,color:"#C8A96A"}}>
      <style>{CSS}</style>
      <div style={{width:40,height:40,border:"3px solid #C8A96A22",borderTopColor:"#C8A96A",borderRadius:"50%",animation:"spin .7s linear infinite"}}/>
      <p style={{fontSize:".85rem",opacity:.6}}>جاري تحضير القائمة...</p>
    </div>
  );
    ...
    <div dir="rtl" style={{fontFamily:"'Noto Kufi Arabic',sans-serif"}}>
      ...
      <div className="noise" style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"radial-gradient(ellipse at 30% 40%,#1f1508,#0f0f0f 60%,#0a0a0a)",position:"relative",overflow:"hidden",textAlign:"center",padding:24}}>
        ...
        <div className="fade-up" style={{maxWidth:400}}>
          ...
          <p style={{fontSize:"clamp(.88rem,3vw,.98rem)",color:"#8B6B4A",marginBottom:46,fontWeight:300,letterSpacing:".5px",lineHeight:1.8}}>{slogan}</p>
  ...
  if (screen==="menu") return (
    <div dir="rtl" style={{fontFamily:"'Noto Kufi Arabic',sans-serif",background:"#0f0f0f",minHeight:"100vh",color:"#E5D3B3",paddingBottom:100}}>
      ...
      <div style={{position:"sticky",top:0,zIndex:30,background:"linear-gradient(180deg,#0f0f0f 85%,transparent)",padding:"13px 16px 11px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:"1px solid #161616"}}>
        <span style={{fontSize:"1.2rem",fontWeight:900,background:"linear-gradient(90deg,#C8A96A,#E5D3B3)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text"}}>{siteName}</span>
      ...
      <div style={{paddingTop:18}}>
        ...
        <FeaturedSlider featured={featured} onCardClick={fp=>{
          if (fp.id==="khanum"||fp.sizes?.length) { setBuilderPizza(fp); setKhanamSize(null); setScreen("khanum"); }
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {pizzasMenu.map(p => (
            <div key={p.id} className={p.comingSoon?"":"card-tap"}
              ...
              <PizzaImg imageUrl={p.imageUrl} label={p.label} style={{width:"100%",height:92,borderRadius:0}}/>
              ...
              {Array.from({length:sliceCount},(_,i)=>{
                ...
                return (
                  <div key={i} className={`slice-cell${isSel?" sel":fid?" fld":""}`} onClick={()=>toggleSlice(i)}>
                    {fid
                      ?<><PizzaImg imageUrl={flavorItem?.flavorImageUrl||""} label="" style={{width:"100%",height:30,borderRadius:4}}/><span style={{fontSize:".5rem",color:"#C8A96A",fontWeight:700,textAlign:"center",lineHeight:1.3}}>{flavorItem?.label}</span></>
    ...
    <div dir="rtl" style={{fontFamily:"'Noto Kufi Arabic',sans-serif",background:"#0d0d0d",minHeight:"100vh",color:"#E5D3B3",paddingBottom:100}}>
      ...
      <div style={{padding:"18px 16px"}}>
        <div style={{background:"#141414",border:"1px solid #C8A96A1a",borderRadius:16,padding:14,marginBottom:20,display:"flex",gap:14,alignItems:"flex-start"}}>
          <PizzaImg imageUrl={builderPizza.imageUrl} label="" style={{width:80,height:80,borderRadius:12,flexShrink:0}}/>
  ...
  if (screen==="pizza_detail" && detailPizza) return (
    <div dir="rtl" style={{fontFamily:"'Noto Kufi Arabic',sans-serif",background:"#0d0d0d",minHeight:"100vh",color:"#E5D3B3",paddingBottom:100}}>
      ...
      <div style={{padding:"18px 16px"}}>
        <PizzaImg imageUrl={detailPizza.imageUrl} label={detailPizza.label} style={{width:"100%",height:200,borderRadius:18,marginBottom:20}}/>
      ...
      <div style={{padding:"18px 16px"}}>
        {cart.length===0?(
          ...
          <>
            ...
            {!canCheckout&&<p style={{textAlign:"center",fontSize:".63rem",color:"#252525",marginTop:8}}>
              {!phoneValid?phone.length===0?"أدخل رقم هاتفك":`الرقم يجب أن يكون 10 أرقام (أدخلت ${phone.length})`:!deliveryType?"اختر طريقة الاستلام":"أدخل موقعك لإتمام الطلب"}
            </p>}
          </>
        )}
      </div>

...
export default function PizzaKhanum() {
  ...
  const [translated,    setTranslated]    = useState({ menu:[], featured:[], sections:[] });

  // AI translation when language changes
  useEffect(() => {
    if (lang === "ar" || loading) return;
    let cancelled = false;
    setTranslating(true);
    (async () => {
      const result = await translateAll(pizzasMenu, featured, menuSections);
      if (!cancelled) {
  ...
  }, [lang, loading]); // eslint-disable-line

  // Use translated data when English
  const displayFeatured = lang === "ar" || !translated.featured.length ? featured : translated.featured;
  const displayMenu = lang === "ar" || !translated.menu.length ? pizzasMenu : translated.menu;
  const displaySectionsArr = lang === "ar" || !translated.sections.length ? menuSections : translated.sections;
          ...
          <div style={{position:"sticky",top:0,zIndex:30,background:"#0f0f0f",borderBottom:"1px solid #161616"}}>
            <div style={{padding:"12px 15px 10px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:6}}>
                {/* Language toggle */}
                <button onClick={toggleLang} style={{background:"#1a1a1a",border:"1px solid #C8A96A2a",borderRadius:8,color:"#C8A96A",padding:"5px 9px",cursor:"pointer",fontSize:".7rem",fontWeight:700,fontFamily:"inherit"}}>
                  {lang==="ar"?"EN":"AR"}
                </button>
                {/* Theme toggle */}
      ...
      <div dir={lang==="ar"?"rtl":"ltr"} style={{fontFamily:"'Noto Kufi Arabic',sans-serif",background:"#0d0d0d",minHeight:"100vh",color:"#E5D3B3",paddingBottom:120}}>
        ...
        <div style={{position:"sticky",top:0,zIndex:20,background:"#0d0d0d",borderBottom:"1px solid #161616",padding:"12px 15px",display:"flex",alignItems:"center",gap:11}}>
          ...
          <div style={{flex:1}}>
            ...
            <p style={{fontSize:".63rem",color:"#8B6B4A"}}>{selectedSlices.size>0?`${selectedSlices.size} ${lang==="ar"?"شريحة محددة — اختر نكهة":"slice selected — choose flavor"}`:`${filledCount}/${sliceCount} ${lang==="ar"?"شرائح":"slices"}`}</p>
          </div>
        ...
        <div style={{padding:"13px 15px 0"}}>
          <div style={{background:"#141414",border:"1px solid #1a1a1a",borderRadius:11,padding:"8px 13px",marginBottom:13,display:"flex",alignItems:"center",gap:9}}>
            ...
            <p style={{fontSize:".7rem",color:"#8B6B4A",lineHeight:1.5}}>{selectedSlices.size===0?(lang==="ar"?"اضغط على شريحة أو أكثر، ثم اختر النكهة":"Tap a slice, then pick a flavor"):`${selectedSlices.size} ${lang==="ar"?"شريحة — اضغط على نكهة":"slice — pick a flavor"}`}</p>
          </div>
          ...
          {bHasExtras&&(<>
            ...
            <p style={{fontSize:".6rem",color:"#C8A96A66",letterSpacing:"2px",marginBottom:11}}>{lang==="ar"?"الإضافات":"Extras"}</p>
          ...
          {bExtraTotal>0&&(
            <div style={{background:"#141414",border:"1px solid #C8A96A1a",borderRadius:11,padding:"10px 13px",marginBottom:14}}>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:".72rem",color:"#8B6B4A",marginBottom:4}}><span>{lang==="ar"?"البيتزا":"Pizza"}</span><span>{fmt(builderPizza.numericPrice)} ل.س</span></div>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:".72rem",color:"#8B6B4A",marginBottom:6}}><span>{lang==="ar"?"الإضافات":"Extras"}</span><span>+ {fmt(bExtraTotal)} ل.س</span></div>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:".84rem",fontWeight:700,color:"#C8A96A",borderTop:"1px solid #1e1e1e",paddingTop:6}}><span>{lang==="ar"?"المجموع":"Total"}</span><span>{fmt(bTotalNow)} ل.س</span></div>
            </div>
          )}
          {filledCount>0&&<div className="pop-in" style={{marginBottom:11}}><button className="btn-gold" onClick={addBuilderToCart} style={{width:"100%",padding:"11px",borderRadius:11,fontSize:".86rem",fontWeight:700,color:"#0f0f0f"}}>{lang==="ar"?"إضافة للطلب":`Add to Order`} — {fmt(bTotalNow)} ل.س</button></div>}
          <p style={{fontSize:".6rem",color:"#8B6B4A",letterSpacing:"2px",marginBottom:9}}>{lang==="ar"?"النكهات المتاحة":"Available Flavors"}</p>
          <FlavorGrid flavors={flavors} onPick={applyFlavor} usedMap={usedMap}/>
          {filledCount>0&&<button onClick={()=>{setSliceFlavors({});setSelectedSlices(new Set());setBuilderExtras({});}} style={{marginTop:11,width:"100%",padding:9,background:"none",border:"1px solid #1a1a1a",borderRadius:11,color:"#333",cursor:"pointer",fontFamily:"inherit",fontSize:".72rem"}}>🔄 {lang==="ar"?"إعادة التعيين":"Reset"}</button>}
        </div>
      </div>
    );
  }
  ...
  if(screen==="khanum"&&builderPizza)return(
    <div dir={lang==="ar"?"rtl":"ltr"} style={{fontFamily:"'Noto Kufi Arabic',sans-serif",background:"#0d0d0d",minHeight:"100vh",color:"#E5D3B3",paddingBottom:100}}>
      ...
      <div style={{position:"sticky",top:0,zIndex:20,background:"#0d0d0d",borderBottom:"1px solid #161616",padding:"12px 15px",display:"flex",alignItems:"center",gap:11}}>
        ...
        <div><h2 style={{fontSize:".92rem",fontWeight:700,color:"#E5D3B3"}}>بيتزا خانم</h2><p style={{fontSize:".62rem",color:"#8B6B4A"}}>{khanamSize?(lang==="ar"?"اختر نكهة المنتصف":"Choose center flavor"):(lang==="ar"?"اختر الحجم أولاً":"Choose size first")}</p></div>
      </div>
      <div style={{padding:"16px 15px"}}>
        ...
        {!khanamSize&&(
          <div>
            <p style={{fontSize:".6rem",color:"#8B6B4A",letterSpacing:"2px",marginBottom:11}}>{lang==="ar"?"اختر الحجم":"Choose size"}</p>
        ...
        {khanamSize&&(
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:11}}>
              <p style={{fontSize:".6rem",color:"#8B6B4A",letterSpacing:"2px"}}>{lang==="ar"?"نكهة المنتصف":"Center flavor"}</p>
              <button onClick={()=>setKhanamSize(null)} style={{fontSize:".66rem",color:"#444",background:"none",border:"none",cursor:"pointer",fontFamily:"inherit"}}>← {lang==="ar"?"تغيير الحجم":"Change size"}</button>
            </div>

...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  @media (max-width:480px){
    .featured-card{flex:0 0 85vw!important}
    .sec-tab{font-size:.7rem!important;padding:5px 10px!important}
    .size-btn{font-size:.72rem!important;padding:9px 4px!important}
  }
  @media (min-width:768px){
    .featured-scroll{padding:0 32px!important}
  }
  /* ── Light mode sync ── */
  [data-theme="light"] .btn-gold{background:var(--gold)!important;color:#fff!important}
  [data-theme="light"] .featured-card{background:var(--bg-card)!important;border-color:var(--border-color)!important}
  [data-theme="light"] .sec-tab{background:var(--bg-secondary)!important;border-color:var(--border-color)!important;color:var(--text-muted)!important}
  [data-theme="light"] .sec-tab.on{background:var(--gold-bg)!important;border-color:var(--gold)!important;color:var(--gold)!important}
  [data-theme="light"] .slice-cell{background:var(--bg-secondary)!important;border-color:var(--border-color)!important}
  [data-theme="light"] .slice-cell.sel{border-color:#4DA6FF!important;background:#E8F4FD!important}
  [data-theme="light"] .slice-cell.fld{border-color:var(--gold)!important}
  [data-theme="light"] .size-btn{background:var(--bg-secondary)!important;border-color:var(--border-color)!important}
  [data-theme="light"] .size-btn.on{background:var(--gold)!important;color:#fff!important}
  [data-theme="light"] .extra-opt{background:var(--bg-secondary)!important;border-color:var(--border-color)!important}
  [data-theme="light"] .extra-opt.sel{background:var(--gold-bg)!important;border-color:var(--gold)!important}
  [data-theme="light"] .flavor-btn{background:var(--bg-card)!important;border-color:var(--border-color)!important}
  [data-theme="light"] .del-btn{background:var(--bg-secondary)!important;border-color:var(--border-color)!important}
  [data-theme="light"] .del-btn.on{background:var(--gold-bg)!important;border-color:var(--gold)!important;color:var(--gold)!important}
  [data-theme="light"] .featured-card.acard{border-color:var(--gold)!important}
`;
...
export default function PizzaKhanum() {
  ...
  // AI translation when language changes or data loads
  useEffect(() => {
  ...
  // Use translated data when English
  ...
  function addKhanamToCart(fid){
    ...
    const eExtra=extrasTotal(builderPizza,builderExtras);
    const eSummary=extrasSummary(builderPizza,builderExtras);
    const details=[`المنتصف: ${f?.label} • الأطراف: جبنة شيدر`,eSummary].filter(Boolean).join(" · ");
    addToCart({label:`بيتزا خانم — ${khanamSize.label}`,size:khanamSize.label,details,priceOld:khanamSize.priceOld,priceNew:khanamSize.priceNew,numericPrice:getNum(khanamSize)+eExtra});
  ...
  /* ════ LOADING ════ */
  if(loading)return(
    <div dir={lang==="ar"?"rtl":"ltr"} style={{fontFamily:"'Noto Kufi Arabic',sans-serif",background:"#0f0f0f",minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:14,color:"#C8A96A"}}>
      <style>{CSS}</style>
  ...
  if(screen==="landing")return(
    <div dir={lang==="ar"?"rtl":"ltr"} style={{fontFamily:"'Noto Kufi Arabic',sans-serif"}}>
      <style>{CSS}</style>
      <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"radial-gradient(ellipse at 30% 40%,#1f1508,#0f0f0f 60%,#0a0a0a)",position:"relative",overflow:"hidden",textAlign:"center",padding:24}}>
        ...
        <div className="fade-up" style={{maxWidth:400}}>
          ...
          <button className="btn-gold" onClick={()=>setScreen("menu")} style={{padding:"15px 48px",borderRadius:"50px",fontSize:"1rem",fontWeight:700,color:"#0f0f0f",letterSpacing:"1px",animation:"glow 3s ease-in-out infinite"}}>{t("start_order", lang)}</button>
        </div>
      </div>
    </div>
  );
  ...
  if(screen==="menu"){
    ...
    return(
      <div dir={lang==="ar"?"rtl":"ltr"} style={{fontFamily:"'Noto Kufi Arabic',sans-serif",background:"#0f0f0f",minHeight:"100vh",color:"#E5D3B3",paddingBottom:100}}>
        <style>{CSS}</style>
          ...
          <div style={{position:"sticky",top:0,zIndex:30,background:"#0f0f0f",borderBottom:"1px solid #161616"}}>
            <div style={{padding:"12px 15px 10px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:6}}>
                {/* Language toggle */}
                <button onClick={toggleLang} style={{background:"#1a1a1a",border:"1px solid #C8A96A2a",borderRadius:8,color:"#C8A96A",padding:"5px 9px",cursor:"pointer",fontSize:".7rem",fontWeight:700,fontFamily:"inherit", opacity:translating?0.5:1}}>
                  {lang==="ar"?"EN":"AR"}
                </button>
                {/* Theme toggle */}
                <button onClick={toggleTheme} style={{background:"#1a1a1a",border:"1px solid #C8A96A2a",borderRadius:8,color:"#C8A96A",width:32,height:32,cursor:"pointer",fontSize:"1rem",display:"flex",alignItems:"center",justifyContent:"center"}}>
                  {theme==="dark"?"🌙":"☀️"}
                </button>
                <button onClick={()=>setScreen("summary")} style={{position:"relative",background:"#1a1a1a",border:"1px solid #C8A96A2a",borderRadius:9,color:"#C8A96A",padding:"7px 12px",cursor:"pointer",fontSize:".76rem",fontFamily:"inherit"}}>
                  🧾 {t("order", lang)}
                  {cart.length>0&&<span style={{position:"absolute",top:-5,left: lang==="ar"?-5:"auto",right: lang==="ar"?"auto":-5,background:"#C8A96A",color:"#0f0f0f",borderRadius:"50%",width:17,height:17,display:"flex",alignItems:"center",justifyContent:"center",fontSize:".55rem",fontWeight:700}}>{cart.length}</span>}
                </button>
              </div>
            </div>
            {/* Translating indicator */}
            {translating && (
              <div style={{height:2,background:"#C8A96A22",overflow:"hidden"}}>
                <div style={{height:"100%",width:"30%",background:"#C8A96A",borderRadius:2,animation:"trSlide 1.2s linear infinite"}}/>
              </div>
            )}
        ...
        <div style={{paddingTop:16}}>
          <p style={{fontSize:".6rem",color:"#C8A96A55",letterSpacing:"3px",padding:"0 15px 9px"}}>⭐ {t("featured_title", lang)}</p>
          <FeaturedSlider featured={displayFeatured} onCardClick={fp=>{
            setBuilderExtras({});
        ...
        <div ref={menuScrollRef} style={{padding:"14px 15px 0"}}>
                ...
                {sec.items.map((p,pi)=>(
                  <div key={p.id||p._id} className={`${p.comingSoon?"":"card-tap"} item-anim`} data-animate="1"
                    ...
                    <div style={{padding:"8px 8px 10px"}}>
                      ...
                      {p.comingSoon?<span style={{fontSize:".55rem",background:"#C8A96A14",color:"#C8A96A77",padding:"2px 7px",borderRadius:19}}>{t("coming_soon", lang)}</span>
          ...
          {unsectioned.length>0&&(
            <div ref={el=>sectionRefs.current["__other__"]=el} data-section-id="__other__" style={{marginBottom:28}}>
              <p style={{fontSize:".6rem",color:"#C8A96A55",letterSpacing:"3px",marginBottom:12}}>🍕 {t("menu", lang)}</p>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
        ...
        {cart.length>0&&(
          <div style={{position:"fixed",bottom:18,left:"50%",transform:"translateX(-50%)",zIndex:40}}>
            <button className="btn-gold pop-in" onClick={()=>setScreen("summary")} style={{padding:"12px 24px",borderRadius:"50px",fontSize:".86rem",fontWeight:700,color:"#0f0f0f",boxShadow:"0 8px 28px #00000099",display:"flex",alignItems:"center",gap:9}}>
              ...
              <span style={{background:"#0f0f0f22",borderRadius:20,padding:"2px 9px",fontSize:".78rem"}}>{fmt(cartTotal)} {t("syp", lang)}</span>
            </button>
          </div>
        )}
      </div>
  ...
  if(screen==="builder"&&builderPizza){
    ...
    return(
      <div dir={lang==="ar"?"rtl":"ltr"} style={{fontFamily:"'Noto Kufi Arabic',sans-serif",background:"#0d0d0d",minHeight:"100vh",color:"#E5D3B3",paddingBottom:120}}>
        <style>{CSS}</style>
        <div style={{position:"sticky",top:0,zIndex:20,background:"#0d0d0d",borderBottom:"1px solid #161616",padding:"12px 15px",display:"flex",alignItems:"center",gap:11}}>
          ...
          <div style={{flex:1}}>
            ...
            <p style={{fontSize:".63rem",color:"#8B6B4A"}}>{selectedSlices.size>0?`${selectedSlices.size} ${t("slice_selected", lang)}`:`${filledCount}/${sliceCount} ${t("slices", lang)}`}</p>
          </div>
        ...
        <div style={{padding:"13px 15px 0"}}>
          <div style={{background:"#141414",border:"1px solid #1a1a1a",borderRadius:11,padding:"8px 13px",marginBottom:13,display:"flex",alignItems:"center",gap:9}}>
            ...
            <p style={{fontSize:".7rem",color:"#8B6B4A",lineHeight:1.5}}>{selectedSlices.size===0?t("tap_slice", lang):`${selectedSlices.size} ${t("slice_pick", lang)}`}</p>
          </div>
          ...
          {bHasExtras&&(<>
            ...
            <p style={{fontSize:".6rem",color:"#C8A96A66",letterSpacing:"2px",marginBottom:11}}>{t("extras", lang)}</p>
            <ExtrasSelector extras={builderPizza.extras} selections={builderExtras} onChange={(gid,val)=>setBuilderExtras(p=>({...p,[gid]:val===null?undefined:val}))}/>
          </>)}
          {bExtraTotal>0&&(
            <div style={{background:"#141414",border:"1px solid #C8A96A1a",borderRadius:11,padding:"10px 13px",marginBottom:14}}>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:".72rem",color:"#8B6B4A",marginBottom:4}}><span>{t("pizza", lang)}</span><span>{fmt(builderPizza.numericPrice)} {t("syp", lang)}</span></div>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:".72rem",color:"#8B6B4A",marginBottom:6}}><span>{t("extras", lang)}</span><span>+ {fmt(bExtraTotal)} {t("syp", lang)}</span></div>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:".84rem",fontWeight:700,color:"#C8A96A",borderTop:"1px solid #1e1e1e",paddingTop:6}}><span>{t("total", lang)}</span><span>{fmt(bTotalNow)} {t("syp", lang)}</span></div>
            </div>
          )}
          {filledCount>0&&<div className="pop-in" style={{marginBottom:11}}><button className="btn-gold" onClick={addBuilderToCart} style={{width:"100%",padding:"11px",borderRadius:11,fontSize:".86rem",fontWeight:700,color:"#0f0f0f"}}>{t("add_to_order", lang)} — {fmt(bTotalNow)} {t("syp", lang)}</button></div>}
          <p style={{fontSize:".6rem",color:"#8B6B4A",letterSpacing:"2px",marginBottom:9}}>{t("available_flavors", lang)}</p>
          <FlavorGrid flavors={flavors} onPick={applyFlavor} usedMap={usedMap}/>
          {filledCount>0&&<button onClick={()=>{setSliceFlavors({});setSelectedSlices(new Set());setBuilderExtras({});}} style={{marginTop:11,width:"100%",padding:9,background:"none",border:"1px solid #1a1a1a",borderRadius:11,color:"#333",cursor:"pointer",fontFamily:"inherit",fontSize:".72rem"}}>🔄 {t("reset", lang)}</button>}
        </div>
      </div>
    );
  }
  ...
  if(screen==="khanum"&&builderPizza)return(
    <div dir={lang==="ar"?"rtl":"ltr"} style={{fontFamily:"'Noto Kufi Arabic',sans-serif",background:"#0d0d0d",minHeight:"100vh",color:"#E5D3B3",paddingBottom:100}}>
      <style>{CSS}</style>
      <div style={{position:"sticky",top:0,zIndex:20,background:"#0d0d0d",borderBottom:"1px solid #161616",padding:"12px 15px",display:"flex",alignItems:"center",gap:11}}>
        <button onClick={()=>{setScreen("menu");setKhanamSize(null);setBuilderExtras({});}} style={{background:"none",border:"none",color:"#C8A96A",cursor:"pointer",fontSize:"1.5rem",lineHeight:1,padding:0}}>‹</button>
        <div><h2 style={{fontSize:".92rem",fontWeight:700,color:"#E5D3B3"}}>{t("pizza", lang)} خانم</h2><p style={{fontSize:".62rem",color:"#8B6B4A"}}>{khanamSize?t("choose_center_flavor", lang):t("choose_size_first", lang)}</p></div>
      </div>
      <div style={{padding:"16px 15px"}}>
        <div style={{background:"#141414",border:"1px solid #C8A96A1a",borderRadius:15,padding:13,marginBottom:18,display:"flex",gap:12,alignItems:"flex-start"}}>
          ...
          <div><p style={{fontSize:".87rem",fontWeight:700,color:"#C8A96A",marginBottom:5}}>{t("pizza", lang)} خانم</p><p style={{fontSize:".68rem",color:"#8B6B4A",lineHeight:1.6}}>{builderPizza.desc}</p></div>
        </div>
        {!khanamSize&&(
          <div>
            <p style={{fontSize:".6rem",color:"#8B6B4A",letterSpacing:"2px",marginBottom:11}}>{t("choose_size", lang)}</p>
            <div style={{display:"flex",gap:9}}>
              {(builderPizza.sizes||[]).map(sz=>(
                <button key={sz.id} className="size-btn" onClick={()=>setKhanamSize(sz)} style={{padding:"14px 8px"}}>
                  ...
                  <div style={{fontSize:".8rem",fontWeight:700,color:"#C8A96A"}}>{sz.priceOld} {t("syp", lang)}</div>
                  <div style={{fontSize:".58rem",color:"#8B6B4A"}}>{sz.priceNew} {t("sp", lang)}</div>
                </button>
              ))}
            </div>
          </div>
        ...
        {khanamSize&&(
          <div>
            {/* Extras for khanum */}
            {(builderPizza.extras||[]).length>0&&(<>
              <p style={{fontSize:".6rem",color:"#C8A96A66",letterSpacing:"2px",marginBottom:11}}>{t("extras", lang)}</p>
              <ExtrasSelector extras={builderPizza.extras} selections={builderExtras} onChange={(gid,val)=>setBuilderExtras(p=>({...p,[gid]:val===null?undefined:val}))}/>
            </>)}
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:11}}>
              <p style={{fontSize:".6rem",color:"#8B6B4A",letterSpacing:"2px"}}>{t("center_flavor", lang)}</p>
              <button onClick={()=>setKhanamSize(null)} style={{fontSize:".66rem",color:"#444",background:"none",border:"none",cursor:"pointer",fontFamily:"inherit"}}>← {t("change_size", lang)}</button>
            </div>
            <FlavorGrid flavors={flavors} onPick={addKhanamToCart}/>
          </div>
        )}
      </div>
    </div>
    ...
    return(
      <div dir={lang==="ar"?"rtl":"ltr"} style={{fontFamily:"'Noto Kufi Arabic',sans-serif",background:"#0d0d0d",minHeight:"100vh",color:"#E5D3B3",paddingBottom:100}}>
        ...
        <div style={{padding:"16px 15px"}}>
          ...
          <p style={{fontSize:".6rem",color:"#C8A96A66",letterSpacing:"2px",marginBottom:11}}>{t("choose_size_prompt", lang)}</p>
          <div style={{display:"flex",gap:9,marginBottom:22}}>
            {sizes.map(sz=>(
              <button key={sz.id} className={`size-btn${detailSize?.id===sz.id?" on":""}`} onClick={()=>setDetailSize(sz)}>
                ...
                <div style={{fontSize:".78rem",fontWeight:700,color:detailSize?.id===sz.id?"#0f0f0f":"#C8A96A"}}>{sz.priceOld||fmt(getNum(sz))} {t("syp", lang)}</div>
                <div style={{fontSize:".58rem",opacity:.7,marginTop:2}}>{sz.priceNew} {t("sp", lang)}</div>
              </button>
            ))}
          </div>
          {hasExtras&&(<>
            <div style={{height:1,background:"#1a1a1a",marginBottom:20}}/>
            <p style={{fontSize:".6rem",color:"#C8A96A66",letterSpacing:"2px",marginBottom:14}}>{t("extras", lang)}</p>
          ...
          {detailSize&&eTotal>0&&(
            <div style={{background:"#141414",border:"1px solid #C8A96A1a",borderRadius:11,padding:"10px 13px",marginBottom:14}}>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:".72rem",color:"#8B6B4A",marginBottom:4}}><span>{t("pizza", lang)}</span><span>{detailSize.priceOld} {t("syp", lang)}</span></div>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:".72rem",color:"#8B6B4A",marginBottom:6}}><span>{t("extras", lang)}</span><span>+ {fmt(eTotal)} {t("syp", lang)}</span></div>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:".84rem",fontWeight:700,color:"#C8A96A",borderTop:"1px solid #1e1e1e",paddingTop:6}}><span>{t("total", lang)}</span><span>{fmt(totalNow)} {t("syp", lang)}</span></div>
            </div>
          )}
          <button className="btn-gold" disabled={!detailSize||!extrasValid} onClick={addDetailToCart} style={{width:"100%",padding:"13px",borderRadius:11,fontSize:".9rem",fontWeight:700,color:"#0f0f0f"}}>
            {!detailSize?t("choose_size_prompt", lang):!extrasValid?t("complete_extras", lang):`${t("add_to_order", lang)} — ${fmt(totalNow)} ${t("syp", lang)}`}
          </button>
        </div>
      </div>
    );
  ...
  if(screen==="summary")return(
    <div dir={lang==="ar"?"rtl":"ltr"} style={{fontFamily:"'Noto Kufi Arabic',sans-serif",background:"#0d0d0d",minHeight:"100vh",color:"#E5D3B3",paddingBottom:50}}>
      <style>{CSS}</style>
      ...
      <div style={{padding:"16px 15px"}}>
        {cart.length===0?(
          <div style={{textAlign:"center",padding:"56px 20px"}}>
            ...
            <p style={{color:"#8B6B4A",fontSize:".85rem",marginBottom:18}}>{t("no_orders", lang)}</p>
            <button className="btn-gold" onClick={()=>setScreen("menu")} style={{padding:"11px 26px",borderRadius:30,fontSize:".83rem",fontWeight:700,color:"#0f0f0f"}}>{t("browse_menu", lang)}</button>
          </div>
        ):(
          <>
            <p style={{fontSize:".6rem",color:"#8B6B4A",letterSpacing:"2px",marginBottom:11}}>{t("orders_label", lang)} ({cart.length})</p>
              ...
              {cart.map(item=>(
                <div key={item.uid} style={{background:"#141414",border:"1px solid #1a1a1a",borderRadius:13,padding:"12px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:9}}>
                    ...
                    <div style={{textAlign:"left",marginRight:9,flexShrink:0}}><div style={{fontSize:".78rem",fontWeight:700,color:"#C8A96A"}}>{fmt(item.numericPrice)} {t("syp", lang)}</div></div>
                  </div>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                    ...
                    <span style={{fontSize:".78rem",color:"#C8A96A",fontWeight:700}}>{fmt(item.numericPrice*item.qty)} {t("syp", lang)}</span>
            ...
            <div style={{borderTop:"1px solid #161616",paddingTop:13,marginBottom:18,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{color:"#8B6B4A",fontSize:".83rem"}}>{t("total_price", lang)}</span>
              <div style={{textAlign:"left"}}><div style={{fontSize:"1.2rem",fontWeight:900,color:"#C8A96A"}}>{fmt(cartTotal)} {t("syp", lang)}</div></div>
            </div>

            {/* طريقة الاستلام */}
            <p style={{fontSize:".6rem",color:errors.delivery?"#ef4444":"#C8A96A66",letterSpacing:"2px",marginBottom:9}}>{errors.delivery?t("delivery_required", lang):t("delivery_method", lang)}</p>
            ...
            {deliveryType==="pickup"&&(
              <div style={{marginBottom:18}}>
                <div style={{background:"#141414",border:"1px solid #C8A96A1a",borderRadius:13,padding:14}}>
                  <p style={{fontSize:".82rem",fontWeight:700,color:"#C8A96A",marginBottom:3}}>{t("branch_name", lang)}</p>
                  <p style={{fontSize:".68rem",color:"#8B6B4A",marginBottom:14}}>{t("branch_address", lang)}</p>
            ...
            {deliveryType==="delivery"&&(
              <div style={{marginBottom:16}}>
                <p style={{fontSize:".6rem",color:errors.location?"#ef4444":"#C8A96A66",letterSpacing:"2px",marginBottom:9}}>{errors.location?t("location_required", lang):`📍 ${t("select_location", lang)}`}</p>
                ...
                {mapCoords&&<div style={{background:"#0d1a0d",border:"1px solid #4CAF5033",borderRadius:9,padding:"7px 12px",marginBottom:9,fontSize:".7rem",color:"#4CAF50"}}>{t("location_confirmed", lang)}</div>}
                <p style={{fontSize:".62rem",color:"#333",marginBottom:7}}>{t("or_write_address", lang)}</p>
            ...
            {/* رقم الهاتف */}
            <p style={{fontSize:".6rem",color:errors.phone?"#ef4444":"#C8A96A66",letterSpacing:"2px",marginBottom:7}}>{errors.phone?t("phone_must_10", lang):t("phone_label", lang)}</p>
            ...
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}>
              <p style={{fontSize:".58rem",color:"#252525"}}>{t("phone_exact", lang)}</p>

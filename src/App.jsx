...
return (
<div style={{ background:“linear-gradient(135deg,#1c1208,#111)”, backgroundImage:“repeating-linear-gradient(45deg,transparent,transparent 7px,rgba(200,169,106,.04) 7px,rgba(200,169,106,.04) 8px)”, display:“flex”, flexDirection:“column”, alignItems:“center”, justifyContent:“center”, gap:5, overflow:“hidden”, …style }}>
...
.card-slide{animation:cardSlide .4s ease forwards}
...
.featured-card{ flex:0 0 78vw;max-width:300px; scroll-snap-align:start;border-radius:20px;overflow:hidden; border:1px solid #252525;transition:border-color .2s,transform .2s;cursor:pointer; background:#121212; }
...
.del-btn{ border:1px solid #252525;border-radius:12px; cursor:pointer;font-family:inherit;font-size:.85rem;font-weight:600; transition:all .2s;flex:1;padding:13px;text-align:center; background:#141414;color:#666; }
...
/* Dot indicators */
.dot-indicator{ display:flex;gap:6px;justify-content:center;padding:10px 0 4px; }
.dot{ width:7px;height:7px;border-radius:50%;transition:all .3s;cursor:pointer; }
.dot.active{ width:20px;border-radius:4px;background:#C8A96A; }
.dot.inactive{ background:#2a2a2a; }
`;

/* ─────────────────── LOCATION PICKER (improved) ──────────────── */
...
tileRef.current = window.L.tileLayer(TILE_LAYERS.street, { attribution:“© OpenStreetMap” }).addTo(map);
...
.addTo(map).bindPopup(“موقعك المحدد ✓”).openPopup();
...
tileRef.current = window.L.tileLayer(TILE_LAYERS[type], {
attribution: type === “street” ? “© OpenStreetMap” : “© Esri”,
...
placeMarker(mapInstanceRef.current, parseFloat(lat), parseFloat(lon));
}
...
placeholder=“ابحث عن موقع… (مثال: دمشق، مزة)”
style={{ flex:1, padding:“9px 12px”, fontSize:”.82rem” }}
/>
<button type=“submit” disabled={searching} style={{ padding:“9px 14px”, background:”#C8A96A”, border:“none”, borderRadius:10, color:”#0f0f0f”, cursor:“pointer”, fontWeight:700, fontFamily:“inherit”, fontSize:”.8rem”, flexShrink:0 }}>
{searching ? “…” : “بحث”}
</button>
</form>
...
```
  ...
  <div style={{ display:"flex", gap:6, marginBottom:8 }}>
    {[["street","🗺 عادية"],["satellite","🛰 قمر صناعي"]].map(([k,lbl]) => (
      <button key={k} onClick={() => switchLayer(k)} style={{ flex:1, padding:"7px", background: mapType===k ? "#C8A96A22" : "#141414", border:`1px solid ${mapType===k ? "#C8A96A" : "#252525"}`, borderRadius:8, color: mapType===k ? "#C8A96A" : "#555", cursor:"pointer", fontFamily:"inherit", fontSize:".75rem", fontWeight:600, transition:"all .2s" }}>
        {lbl}
      </button>
    ))}
  </div>
...
/* ─────────────── FEATURED SLIDER with auto-scroll ──────────────── */
...
return (
<div>
<div className="featured-scroll" ref={scrollRef} onScrollEnd={handleScroll} onTouchEnd={handleScroll}>
{featured.map((fp, i) => (
<div key={fp.id} className={`featured-card${i === active ? " card-slide" : ""}`}
...
```
  ...
  <div className="dot-indicator">
    {featured.map((_, i) => (
      <div key={i} className={`dot ${i === active ? "active" : "inactive"}`}
...
// ── بيانات من Admin (localStorage) ──────────────────────────
const [featured,    setFeatured]    = useState(() => loadLS(“admin_featured”, DEFAULT_FEATURED));
const [pizzasMenu,  setPizzasMenu]  = useState(() => loadLS(“admin_menu”,     DEFAULT_MENU));

// sync عند focus (لو الأدمن عدّل في tab ثاني)
useEffect(() => {
function sync() {
setFeatured(loadLS(“admin_featured”, DEFAULT_FEATURED));
setPizzasMenu(loadLS(“admin_menu”, DEFAULT_MENU));
}
...
function addToCart(item)     { setCart(prev => […prev, { …item, qty:1, uid:Date.now()+Math.random() }]); }
function updateQty(uid, d)   { setCart(prev => prev.map(i => i.uid===uid ? { …i, qty:Math.max(1,i.qty+d) } : i)); }
function removeItem(uid)     { setCart(prev => prev.filter(i => i.uid!==uid)); }
function toggleSlice(idx)    { setSelectedSlices(prev => { const n=new Set(prev); n.has(idx)?n.delete(idx):n.add(idx); return n; }); }

function applyFlavor(fid) {
if (!selectedSlices.size) return;
setSliceFlavors(prev => { const n={…prev}; selectedSlices.forEach(i=>{n[i]=fid;}); return n; });
...
const msg = [“مرحباً بيتزا خانم 🍕”,””,“📋 تفاصيل الطلب:”,lines,””,`💰 المجموع: ${fmt(cartTotal)} ل.س / ${fmt(cartTotal/100)} ل.ج`,””,`🚗 طريقة الاستلام: ${deliveryType==="pickup"?"استلام من الفرع":"توصيل"}`,deliveryType===“delivery”?`📍 الموقع: ${locationTxt}${mapCoords?`\n🗺 خريطة: https://maps.google.com/?q=${mapCoords.lat},${mapCoords.lng}`:""}`:””,” “,`📞 رقم التواصل: ${phone}`].filter(Boolean).join(”\n”);
...
return (
<div key={f.id} className={`flavor-btn${f.comingSoon?" cs":cnt>0?" has-fl":""}`} onClick={()=>{ if(f.comingSoon)return; onPick(f.id); }}>
...
{FLOATERS.map((f,i)=>(<div key={i} style={{ position:“absolute”, fontSize:“1.8rem”, opacity:.17, left:f.l, top:f.t, animation:`floatUp ${f.d}s ease-in-out ${f.dl}s infinite`, pointerEvents:“none”, filter:“blur(.4px)” }}>{f.e}</div>))}
...
<h1 style={{ fontSize:“clamp(2.8rem,9vw,4.2rem)”, fontWeight:900, background:“linear-gradient(135deg,#C8A96A,#E5D3B3,#8B6B4A,#C8A96A)”, backgroundSize:“200% auto”, WebkitBackgroundClip:“text”, WebkitTextFillColor:“transparent”, backgroundClip:“text”, animation:“shimmer 4s linear infinite”, lineHeight:1.1, marginBottom:14 }}>بيتزا خانم</h1>
<p style={{ fontSize:“clamp(.9rem,3vw,1rem)”, color:”#8B6B4A”, marginBottom:48, fontWeight:300, letterSpacing:”.5px”, lineHeight:1.8 }}>كُل لتعيش<span style={{ color:”#C8A96A33”, margin:“0 10px” }}>·</span>وعِش لأجل البيتزا</p>
...
<p style={{ fontSize:”.66rem”, color:”#8B6B4A” }}>{selectedSlices.size>0?`${selectedSlices.size} شريحة محددة — اختر نكهة من الأسفل`:`${filledCount}/${sliceCount} شرائح`}</p>
</div>
<div style={{ display:“flex”, gap:4 }}>
{Array.from({length:sliceCount},(*,i)=>(
...
<p style={{ fontSize:”.72rem”, color:”#8B6B4A”, lineHeight:1.5 }}>{selectedSlices.size===0?“اضغط على شريحة أو أكثر لتحديدها، ثم اختر النكهة”:`${selectedSlices.size} شريحة محددة — اضغط على نكهة لتطبيقها`}</p>
</div>
...
const isSel=selectedSlices.has(i); const isFilled=!!fid;
return (
<div key={i} className={`slice-cell${isSel?" selected":isFilled?" flavored":""}`} onClick={()=>toggleSlice(i)}>
{isFilled
? <><PizzaImg label=”” style={{ width:“100%”, height:28, borderRadius:4 }} /><span style={{ fontSize:”.52rem”, color:”#C8A96A”, fontWeight:700, textAlign:“center”, lineHeight:1.3 }}>{flbl}</span></>
...
<div><h2 style={{ fontSize:”.95rem”, fontWeight:700, color:”#E5D3B3” }}>بيتزا خانم</h2><p style={{ fontSize:”.66rem”, color:”#8B6B4A” }}>{khanamSize?“اختر نكهة المنتصف”:“اختر الحجم أولاً”}</p></div>
</div>
...
<div><p style={{ fontSize:”.88rem”, fontWeight:700, color:”#C8A96A”, marginBottom:6 }}>بيتزا خانم</p><p style={{ fontSize:”.7rem”, color:”#8B6B4A”, lineHeight:1.6 }}>{builderPizza.desc}</p></div>
</div>
{!khanamSize && (<div className="slide-in"><p style={{ fontSize:”.65rem”, color:”#8B6B4A”, letterSpacing:“2px”, marginBottom:12 }}>اختر الحجم</p><div style={{ display:“flex”, gap:10 }}>{builderPizza.sizes?.map(sz=>(<button key={sz.id} className=“size-btn” onClick={()=>setKhanamSize(sz)} style={{ padding:“16px 8px” }}><div style={{ fontSize:“1.4rem”, marginBottom:6 }}>{sz.id===“sm”?“🔸”:“🔶”}</div><div style={{ color:”#E5D3B3”, marginBottom:5 }}>{sz.label}</div><div style={{ fontSize:”.82rem”, fontWeight:700, color:”#C8A96A” }}>{sz.priceOld} ل.س</div><div style={{ fontSize:”.62rem”, color:”#8B6B4A” }}>{sz.priceNew} ل.ج</div></button>))}</div></div>)}
{khanamSize && (<div className="slide-in"><div style={{ display:“flex”, alignItems:“center”, justifyContent:“space-between”, marginBottom:14 }}><p style={{ fontSize:”.65rem”, color:”#8B6B4A”, letterSpacing:“2px” }}>نكهة المنتصف</p><button onClick={()=>setKhanamSize(null)} style={{ fontSize:”.68rem”, color:”#555”, background:“none”, border:“none”, cursor:“pointer”, fontFamily:“inherit” }}>← تغيير الحجم</button></div><FlavorGrid onPick={addKhanamToCart} /></div>)}
</div>
</div>
);
...
{SIZES_REGULAR.map(sz=>(<button key={sz.id} className={`size-btn${detailSize?.id===sz.id?" active":""}`} onClick={()=>setDetailSize(sz)}><div style={{ marginBottom:4, fontWeight:700 }}>{sz.label}</div><div style={{ fontSize:”.8rem”, fontWeight:700, color:detailSize?.id===sz.id?”#0f0f0f”:”#C8A96A” }}>{sz.priceOld} ل.س</div><div style={{ fontSize:”.6rem”, opacity:.75, marginTop:2 }}>{sz.priceNew} ل.ج</div></button>))}
</div>
<button className=“btn-gold” disabled={!detailSize} onClick={addDetailToCart} style={{ width:“100%”, padding:“14px”, borderRadius:12, fontSize:”.92rem”, fontWeight:700, color:”#0f0f0f” }}>{detailSize?`إضافة للطلب — ${detailSize.priceOld} ل.س`:“اختر الحجم أولاً”}</button>
</div>
</div>
);
...
```
      ...
      {deliveryType==="delivery" && (
        <div className="pop-in" style={{ marginBottom:18 }}>
          ...
          <textarea className={errors.location?"err-field":""} rows={2} placeholder="المحافظة، الحي، الشارع، أقرب نقطة دالة..." value={mapCoords?"":locationTxt} onChange={e=>{setLocationTxt(e.target.value);setMapCoords(null);setErrors(ev=>({...ev,location:false}));}} style={{ resize:"none" }} />
        </div>
      )}
      ...
      <input type="tel" className={errors.phone||(phone.length>0&&phone.length!==10)?"err-field":""} placeholder="09xxxxxxxx" value={phone} maxLength={10} onChange={e=>{const val=e.target.value.replace(/\D/g,"");setPhone(val);setErrors(ev=>({...ev,phone:false}));}} style={{ marginBottom:4 }} />
      ...
      {phone.length>0&&phone.length<10&&(<p style={{ fontSize:".62rem", color:"#ef4444", marginBottom:12, marginTop:-12 }}>⚠ الرقم ناقص — أدخل {10-phone.length} أرقام إضافية</p>)}
      ...
      {!canCheckout&&(<p style={{ textAlign:"center", fontSize:".65rem", color:"#333", marginTop:8 }}>{!phoneValid?phone.length===0?"أدخل رقم هاتفك لإتمام الطلب":`الرقم يجب أن يكون 10 أرقام (أدخلت ${phone.length})`:!deliveryType?"اختر طريقة الاستلام":"أدخل موقعك لإتمام الطلب"}</p>)}
    </>)}
  </div>
</div>
```

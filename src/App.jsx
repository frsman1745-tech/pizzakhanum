...
function getImg(id)  { return localStorage.getItem(`pizza_img_${id}`) || null; }
...
/* Shows real image if uploaded via Admin, otherwise placeholder */
function PizzaImg({ id, label, style, imgStyle = {} }) {
  const [src, setSrc] = useState(() => id ? getImg(id) : null);
  useEffect(() => { if (id) setSrc(getImg(id)); }, [id]);

  if (src) {
    return (
      <div style={{ overflow:"hidden", flexShrink:0, ...style }}>
        <img src={src} alt={label||""} style={{ width:"100%", height:"100%", objectFit:"cover", display:"block", ...imgStyle }}/>
      </div>
    );
  }
  return (
    <div style={{ background:"linear-gradient(135deg,#1c1208,#111)", backgroundImage:"repeating-linear-gradient(45deg,transparent,transparent 7px,rgba(200,169,106,.04) 7px,rgba(200,169,106,.04) 8px)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:5, overflow:"hidden", flexShrink:0, ...style }}>
      <div style={{ width:30, height:30, borderRadius:"50%", border:"1px dashed #C8A96A33", display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, opacity:.35 }}>🍕</div>
      {label && <span style={{ fontSize:".5rem", color:"#C8A96A33", textAlign:"center", padding:"0 6px", lineHeight:1.3 }}>{label}</span>}
    </div>
  );
}
...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .card-tap:hover{transform:translateY(-3px) scale(1.015);box-shadow:0 10px 36px rgba(200,169,106,.15)}
  ...
  .featured-card{flex:0 0 78vw;max-width:300px;scroll-snap-align:start;border-radius:20px;overflow:hidden;border:1px solid #252525;transition:border-color .25s,transform .25s;cursor:pointer;background:#121212}
  .featured-card:hover{border-color:#C8A96A55;transform:scale(1.02)}
  .featured-card:active{transform:scale(.98)}
  .featured-card.active-card{border-color:#C8A96A44}
  ...
  .dot.off{background:#232323}

  .slice-cell{cursor:pointer;border:1.5px solid #242424;border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:62px;padding:6px 4px;gap:3px;background:linear-gradient(135deg,#1c0e05,#120a02);transition:all .17s}
  .slice-cell:hover{border-color:#C8A96A44;background:#1f1008}
  .slice-cell.sel{border-color:#4DA6FF!important;background:#06111f!important;box-shadow:0 0 12px #4DA6FF44}
  .slice-cell.fld{border-color:#C8A96A66;background:linear-gradient(135deg,#2a1508,#1a0e05)}

  .flavor-btn{cursor:pointer;border:1px solid #1e1e1e;background:#141414;border-radius:12px;overflow:hidden;transition:all .17s}
  .flavor-btn:hover:not(.cs){border-color:#C8A96A55;background:#1a1408}
  .flavor-btn.hfl{border-color:#C8A96A55}
  .flavor-btn.cs{cursor:not-allowed;opacity:.45}

  .size-btn{flex:1;padding:12px 6px;border-radius:12px;cursor:pointer;font-family:inherit;font-size:.8rem;font-weight:600;border:1px solid #252525;background:#141414;color:#E5D3B3;transition:all .2s;text-align:center}
  .size-btn:hover{border-color:#C8A96A55}
  .size-btn.on{background:#C8A96A;border-color:#C8A96A;color:#0f0f0f}

  .del-btn{border:1px solid #252525;border-radius:12px;cursor:pointer;font-family:inherit;font-size:.85rem;font-weight:600;transition:all .2s;flex:1;padding:13px;text-align:center;background:#141414;color:#555}
  .del-btn.on{border-color:#C8A96A;background:#1c1308;color:#C8A96A}
  .del-btn:hover:not(.on){border-color:#C8A96A33}
...
function LocationPicker({ onSelect }) {
  ...
  useEffect(() => {
    if (mapRef2.current || !window.L) return;
    const map = window.L.map(mapRef.current, { zoomControl:true }).setView([33.51,36.29], 12);
  ...
  function switchLayer(t) {
    if (!mapRef2.current || t===mapType) return;
    if (tileRef.current) tileRef.current.remove();
    tileRef.current = window.L.tileLayer(TILES[t],{attribution:t==="street"?"© OpenStreetMap":"© Esri"}).addTo(mapRef2.current);
    setMapType(t);
  }
  async function doSearch(e) {
    e.preventDefault();
    if (!search.trim()||!mapRef2.current) return;
    setBusy(true);
    try {
      const r = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(search)}&limit=1`);
      const d = await r.json();
      if (d[0]) { const {lat,lon}=d[0]; mapRef2.current.flyTo([lat,lon],15,{duration:1.2}); place(mapRef2.current,+lat,+lon); }
    } finally { setBusy(false); }
  }
  return (
    <div>
      <form onSubmit={doSearch} style={{display:"flex",gap:8,marginBottom:8}}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="ابحث عن موقع... (مثال: دمشق، مزة)" style={{flex:1,padding:"9px 12px",fontSize:".82rem"}}/>
        <button type="submit" disabled={busy} style={{padding:"9px 14px",background:"#C8A96A",border:"none",borderRadius:10,color:"#0f0f0f",cursor:"pointer",fontWeight:700,fontFamily:"inherit",fontSize:".8rem",flexShrink:0}}>{busy?"...":"بحث"}</button>
      </form>
      <div style={{display:"flex",gap:6,marginBottom:8}}>
        {[["street","🗺 عادية"],["satellite","🛰 قمر صناعي"]].map(([k,l])=>(
          <button key={k} onClick={()=>switchLayer(k)} style={{flex:1,padding:"7px",background:mapType===k?"#C8A96A22":"#141414",border:`1px solid ${mapType===k?"#C8A96A":"#252525"}`,borderRadius:8,color:mapType===k?"#C8A96A":"#555",cursor:"pointer",fontFamily:"inherit",fontSize:".75rem",fontWeight:600,transition:"all .2s"}}>{l}</button>
        ))}
      </div>
      <div ref={mapRef} style={{width:"100%",height:260,borderRadius:14,border:"1px solid #C8A96A33",overflow:"hidden",marginBottom:10}}/>
    </div>
  );
}
  ...
  return (
    <div>
      <div className="featured-scroll" ref={scrollRef} onScroll={onScroll}>
        {featured.map((fp, i) => (
          <div key={fp.id} className={`featured-card${i===active?" active-card":""}`} onClick={()=>onCardClick(fp)}>
            <PizzaImg id={fp.id} label={fp.label} style={{width:"100%",height:160,borderRadius:0}}/>
            <div style={{padding:"12px 13px 14px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                ...
                <div style={{textAlign:"left",flexShrink:0,marginRight:8}}>
                  {fp.priceOld
                    ? <><div style={{fontSize:".82rem",fontWeight:900,color:"#C8A96A",whiteSpace:"nowrap"}}>{fp.priceOld} ل.س</div><div style={{fontSize:".6rem",color:"#8B6B4A"}}>{fp.priceNew} ل.ج</div></>
      ...
      <div className="dot-wrap">
        {featured.map((_,i)=>(
          <button key={i} className={`dot ${i===active?"on":"off"}`} onClick={()=>manualNav(i)}/>
        ))}
      </div>
    </div>
  );
...
export default function PizzaKhanum() {
  ...
  // Sync when tab regains focus (after editing in admin tab)
  useEffect(()=>{
    function sync() {
      setFeatured(ls("admin_featured", DEFAULT_FEATURED));
      setPizzasMenu(ls("admin_menu",   DEFAULT_MENU));
    }
  ...
  function checkout() {
    ...
    const whatsapp = ls("site_whatsapp","963998950904");
    const lines = cart.map(i=>`• ${i.label}${i.size?` (${i.size})`:""} × ${i.qty}\n  ${i.details}\n  السعر: ${i.priceOld} ل.س`).join("\n\n");
    const msg = ["مرحباً بيتزا خانم 🍕","","📋 تفاصيل الطلب:",lines,"",`💰 المجموع: ${fmt(cartTotal)} ل.س`,`🚗 ${deliveryType==="pickup"?"استلام من الفرع":"توصيل"}`,deliveryType==="delivery"?`📍 ${locationTxt}${mapCoords?`\n🗺 https://maps.google.com/?q=${mapCoords.lat},${mapCoords.lng}`:""}`:""," ",`📞 ${phone}`].filter(Boolean).join("\n");
    ...
    return (
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8}}>
        {FLAVORS.map(f=>{
          ...
          // find matching menu item for its image
          const menuItem = pizzasMenu.find(p=>p.id===f.id);
          return (
            <div key={f.id} className={`flavor-btn${f.comingSoon?" cs":cnt>0?" hfl":""}`} onClick={()=>{ if(f.comingSoon)return; onPick(f.id); }}>
              <PizzaImg id={menuItem?.id||f.id} label="" style={{width:"100%",height:70,borderRadius:0}}/>
              <div style={{padding:"7px 10px 9px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                ...
                {f.comingSoon&&<span style={{fontSize:".55rem",background:"#C8A96A22",color:"#C8A96A88",padding:"1px 6px",borderRadius:10}}>قريباً</span>}
    ...
    <div dir="rtl" style={{fontFamily:"'Noto Kufi Arabic',sans-serif"}}>
      ...
      <div className="noise" style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"radial-gradient(ellipse at 30% 40%,#1f1508,#0f0f0f 60%,#0a0a0a)",position:"relative",overflow:"hidden",textAlign:"center",padding:24}}>
        ...
        <div className="fade-up" style={{maxWidth:400}}>
          <div style={{width:108,height:108,borderRadius:"50%",border:"2px solid #C8A96A44",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 26px",animation:"glow 3s ease-in-out infinite",fontSize:"3rem"}}>🍕</div>
          ...
          <p style={{fontSize:"clamp(.88rem,3vw,.98rem)",color:"#8B6B4A",marginBottom:46,fontWeight:300,letterSpacing:".5px",lineHeight:1.8}}>
            {ls("site_slogan","كُل لتعيش · وعِش لأجل البيتزا")}
          </p>
          ...
          <p style={{marginTop:16,fontSize:".66rem",color:"#1e1e1e",letterSpacing:"3px"}}>PIZZA KHANUM • منذ 2020</p>
        </div>
      </div>
    </div>
  );
  ...
  if (screen==="menu") return (
    <div dir="rtl" style={{fontFamily:"'Noto Kufi Arabic',sans-serif",background:"#0f0f0f",minHeight:"100vh",color:"#E5D3B3",paddingBottom:100}}>
      ...
      {/* sticky header */}
      <div style={{position:"sticky",top:0,zIndex:30,background:"linear-gradient(180deg,#0f0f0f 85%,transparent)",padding:"13px 16px 11px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:"1px solid #181818"}}>
        <span style={{fontSize:"1.2rem",fontWeight:900,background:"linear-gradient(90deg,#C8A96A,#E5D3B3)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text"}}>
          {ls("site_name","بيتزا خانم")}
        </span>
        <button onClick={()=>setScreen("summary")} style={{position:"relative",background:"#1a1a1a",border:"1px solid #C8A96A33",borderRadius:10,color:"#C8A96A",padding:"8px 13px",cursor:"pointer",fontSize:".78rem",fontFamily:"inherit"}}>
          🧾 الطلب
      ...
      <div style={{paddingTop:18}}>
        <p style={{fontSize:".63rem",color:"#C8A96A66",letterSpacing:"3px",padding:"0 16px 10px"}}>⭐ العروض المميّزة</p>
      ...
      <div style={{padding:"24px 16px 0"}}>
        <p style={{fontSize:".63rem",color:"#C8A96A66",letterSpacing:"3px",marginBottom:12}}>🍕 قائمة البيتزا</p>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {pizzasMenu.map(p=>(
            <div key={p.id} className={p.comingSoon?"":"card-tap"}
              style={{background:"#131313",border:"1px solid #1c1c1c",borderRadius:14,overflow:"hidden",cursor:p.comingSoon?"default":"pointer",opacity:p.comingSoon?0.55:1}}
              onClick={()=>{ if(p.comingSoon)return; setDetailPizza(p); setDetailSize(null); setScreen("pizza_detail"); }}>
              <PizzaImg id={p.id} label={p.label} style={{width:"100%",height:92,borderRadius:0}}/>
        ...
        <div style={{position:"sticky",top:0,zIndex:20,background:"#0d0d0d",borderBottom:"1px solid #1a1a1a",padding:"13px 16px",display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{display:"flex",gap:4}}>
            {Array.from({length:sliceCount},(_,i)=>(
              <div key={i} style={{width:7,height:7,borderRadius:"50%",background:sliceFlavors[i]?"#C8A96A":"#1e1e1e",transition:"background .3s"}}/>
            ))}
          </div>
        </div>
        <div style={{padding:"14px 16px 0"}}>
          <div style={{background:"#141414",border:"1px solid #1e1e1e",borderRadius:12,padding:"9px 14px",marginBottom:14,display:"flex",alignItems:"center",gap:10}}>
            <span style={{fontSize:"1.1rem"}}>{selectedSlices.size===0?"☝️":"🎨"}</span>
            <p style={{fontSize:".72rem",color:"#8B6B4A",lineHeight:1.5}}>{selectedSlices.size===0?"اضغط على شريحة أو أكثر، ثم اختر النكهة":`${selectedSlices.size} شريحة — اضغط على نكهة لتطبيقها`}</p>
          </div>
          <div style={{background:"linear-gradient(135deg,#1c1008,#100a04)",border:"2px solid #C8A96A22",borderRadius:16,padding:12,marginBottom:16}}>
            <p style={{fontSize:".58rem",color:"#C8A96A33",textAlign:"center",marginBottom:10,letterSpacing:"2px"}}>{builderPizza.label} · اضغط لتحديد الشرائح</p>
            <div style={{display:"grid",gridTemplateColumns:`repeat(${cols},1fr)`,gap:6}}>
              {Array.from({length:sliceCount},(_,i)=>{
                const fid=sliceFlavors[i]; const flbl=fid?FLAVORS.find(f=>f.id===fid)?.label:null;
                const isSel=selectedSlices.has(i);
                const menuItem=pizzasMenu.find(p=>p.id===fid);
                return (
                  <div key={i} className={`slice-cell${isSel?" sel":fid?" fld":""}`} onClick={()=>toggleSlice(i)}>
                    {fid
                      ? <><PizzaImg id={menuItem?.id||fid} label="" style={{width:"100%",height:30,borderRadius:4}}/><span style={{fontSize:".5rem",color:"#C8A96A",fontWeight:700,textAlign:"center",lineHeight:1.3}}>{flbl}</span></>
                      : <><div style={{width:18,height:18,borderRadius:4,border:`1.5px ${isSel?"solid #4DA6FF":"dashed #2a2a2a"}`,display:"flex",alignItems:"center",justifyContent:"center"}}>{isSel&&<div style={{width:8,height:8,borderRadius:"50%",background:"#4DA6FF"}}/>}</div><span style={{fontSize:".5rem",color:"#2a2a2a"}}>{i+1}</span></>
                    }
                  </div>
                );
              })}
          ...
          {filledCount>0&&<button onClick={()=>{setSliceFlavors({});setSelectedSlices(new Set());}} style={{marginTop:12,width:"100%",padding:10,background:"none",border:"1px solid #1e1e1e",borderRadius:12,color:"#444",cursor:"pointer",fontFamily:"inherit",fontSize:".75rem"}}>🔄 إعادة التعيين</button>}
        </div>
      </div>
    );
  }
  ...
  if (screen==="khanum"&&builderPizza) return (
    <div dir="rtl" style={{fontFamily:"'Noto Kufi Arabic',sans-serif",background:"#0d0d0d",minHeight:"100vh",color:"#E5D3B3",paddingBottom:100}}>
      ...
      <div style={{padding:"18px 16px"}}>
        <div style={{background:"#141414",border:"1px solid #C8A96A22",borderRadius:16,padding:14,marginBottom:20,display:"flex",gap:14,alignItems:"flex-start"}}>
          <PizzaImg id={builderPizza.id} label="" style={{width:80,height:80,borderRadius:12,flexShrink:0}}/>
  ...
  if (screen==="pizza_detail"&&detailPizza) return (
    <div dir="rtl" style={{fontFamily:"'Noto Kufi Arabic',sans-serif",background:"#0d0d0d",minHeight:"100vh",color:"#E5D3B3",paddingBottom:100}}>
      ...
      <div style={{padding:"18px 16px"}}>
        <PizzaImg id={detailPizza.id} label={detailPizza.label} style={{width:"100%",height:200,borderRadius:18,marginBottom:20}}/>
        ...
        <p style={{fontSize:".63rem",color:"#C8A96A77",letterSpacing:"2px",marginBottom:12}}>اختر الحجم</p>
    ...
    <div dir="rtl" style={{fontFamily:"'Noto Kufi Arabic',sans-serif",background:"#0d0d0d",minHeight:"100vh",color:"#E5D3B3",paddingBottom:50}}>
      ...
      <div style={{padding:"18px 16px"}}>
        {cart.length===0?(
              ...
              <div key={item.uid} style={{background:"#141414",border:"1px solid #1e1e1e",borderRadius:14,padding:"13px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                  <div style={{flex:1}}>
                    ...
                    <p style={{fontSize:".62rem",color:"#555",marginTop:3,lineHeight:1.4,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{item.details}</p>
                  </div>
          ...
          <div style={{borderTop:"1px solid #1a1a1a",paddingTop:14,marginBottom:20,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <div style={{textAlign:"left"}}>
              ...
              <div style={{fontSize:".64rem",color:"#8B6B4A"}}>{fmt(cartTotal/100)} ل.ج</div>
            </div>
          </div>

          <p style={{fontSize:".63rem",color:errors.delivery?"#ef4444":"#C8A96A77",letterSpacing:"2px",marginBottom:10}}>{errors.delivery?"⚠ هذا الحقل إلزامي":"طريقة الاستلام *"}</p>
          ...
          {deliveryType==="delivery"&&(
            <div className="pop-in" style={{marginBottom:16}}>
              <p style={{fontSize:".63rem",color:errors.location?"#ef4444":"#C8A96A77",letterSpacing:"2px",marginBottom:10}}>{errors.location?"⚠ حدد موقعك أو اكتب العنوان":"📍 حدد موقعك على الخريطة *"}</p>
              ...
              {mapCoords&&<div style={{background:"#0d1a0d",border:"1px solid #4CAF5044",borderRadius:10,padding:"8px 13px",marginBottom:10,fontSize:".72rem",color:"#4CAF50"}}>✓ تم تحديد الموقع — يمكنك سحب الدبوس لتعديله</div>}
              <p style={{fontSize:".62rem",color:"#444",marginBottom:8}}>أو اكتب العنوان يدوياً</p>
          ...
          <p style={{fontSize:".63rem",color:errors.phone?"#ef4444":"#C8A96A77",letterSpacing:"2px",marginBottom:8}}>{errors.phone?"⚠ رقم الهاتف إلزامي":"رقم الهاتف للتواصل *"}</p>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:16}}>
            <p style={{fontSize:".6rem",color:"#2e2e2e"}}>يجب أن يكون 10 أرقام بالضبط</p>
            <span style={{fontSize:".6rem",fontWeight:700,color:phone.length===0?"#2a2a2a":phone.length===10?"#4CAF50":"#ef4444"}}>{phone.length}/10</span>
          </div>
          {phone.length>0&&phone.length<10&&<p style={{fontSize:".62rem",color:"#ef4444",marginBottom:12,marginTop:-10}}>⚠ الرقم ناقص — أدخل {10-phone.length} أرقام إضافية</p>}
          ...
          {!canCheckout&&<p style={{textAlign:"center",fontSize:".64rem",color:"#2e2e2e",marginTop:8}}>{!phoneValid?phone.length===0?"أدخل رقم هاتفك":`الرقم يجب أن يكون 10 أرقام (أدخلت ${phone.length})`:!deliveryType?"اختر طريقة الاستلام":"أدخل موقعك لإتمام الطلب"}</p>}
        </>)}
      </div>
    </div>
  );

...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  @keyframes itemIn{from{opacity:0;transform:translateY(28px) scale(.97)}to{opacity:1;transform:translateY(0) scale(1)}}
  .item-anim{opacity:0;transition:opacity .45s ease,transform .45s ease}
  .item-anim.visible{opacity:1;transform:translateY(0) scale(1)!important}
`;
...
export default function PizzaKhanum() {
  ...
  /* ── IntersectionObserver: (1) scroll spy for tabs  (2) card fade-in ── */
  useEffect(()=>{
    if(screen!=="menu") return;

    // (1) Section scroll spy — updates active tab
    ...
    // (2) Card fade-in — only animate when card enters viewport
    const cardObs = new IntersectionObserver(entries=>{
      entries.forEach(e=>{
        if(e.isIntersecting){
          e.target.classList.add("visible");
          cardObs.unobserve(e.target); // animate once only
        }
      });
    },{threshold:0.08});
              ...
              {cart.map(item=>(
                <div key={item.uid} style={{background:"#141414",border:"1px solid #1a1a1a",borderRadius:13,padding:"12px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:9}}>
                    ...
                    <div style={{textAlign:"left",marginRight:9,flexShrink:0}}><div style={{fontSize:".78rem",fontWeight:700,color:"#C8A96A"}}>{item.fmt(numericPrice)} ل.س</div></div>
                  </div>

...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  @keyframes itemIn{from{opacity:0;transform:translateY(22px)}to{opacity:1;transform:translateY(0)}}
  .item-anim{opacity:0;animation:itemIn .42s ease forwards}
`;
    ...
    function tryInit() {
      ...
      try {
        ...
        const icon = window.L.divIcon({
          html: `<div style="
            background:linear-gradient(135deg,#C8A96A,#8B6B4A);
        ...
        window.L.marker([BRANCH.lat, BRANCH.lng], { icon })
          .addTo(map)
          .bindPopup(`<div style="font-family:'Noto Kufi Arabic',sans-serif;direction:rtl;font-size:.82rem;text-align:right;padding:4px 2px"><strong style="color:#C8A96A">${BRANCH.name}</strong><br><span style="color:#666;font-size:.72rem">اضغط "ذهاب" لفتح الخريطة</span></div>`)
          .openPopup();
  ...
  return (
    <div>
      <div ref={divRef} style={{
        ...
        background: "#111", marginBottom: 10,
      }}>
        {!ready && (
          <div style={{height:"100%",display:"flex",alignItems:"center",justifyContent:"center",color:"#333",fontSize:".78rem",gap:8}}>
            <div style={{width:16,height:16,border:"2px solid #C8A96A33",borderTopColor:"#C8A96A",borderRadius:"50%",animation:"spin .7s linear infinite"}}/>
            جاري تحميل الخريطة...
          </div>
        )}
      </div>
      <a
        ...
        style={{
          ...
          color: "#4CAF50", fontSize: ".84rem", fontWeight: 700,
          fontFamily: "inherit", textDecoration: "none",
          transition: "background .2s",
        }}
        onMouseEnter={e => e.currentTarget.style.background="linear-gradient(135deg,#1e441e,#0f200f)"}
        onMouseLeave={e => e.currentTarget.style.background="linear-gradient(135deg,#1a3a1a,#0d1a0d)"}
      >
...
function FeaturedSlider({ featured, onCardClick }) {
  ...
  return(
    <div>
      ...
      <div className="dot-wrap">{featured.map((_,i)=><button key={i} className={`dot ${i===active?"on":"off"}`} onClick={()=>manualNav(i)}/>)}</div>
    </div>
  );
}
...
export default function PizzaKhanum() {
  ...
  /* scroll spy removed — tabs are static, user scrolls manually */
  ...
  function addDetailToCart(){
    ...
    addToCart({label:detailPizza.label,size:detailSize.label,details,priceOld:detailSize.priceOld,priceNew:detailSize.priceNew,numericPrice:totalNum});
  ...
  if(screen==="menu"){
    ...
    return(
      <div dir="rtl" style={{fontFamily:"'Noto Kufi Arabic',sans-serif",background:"#0f0f0f",minHeight:"100vh",color:"#E5D3B3",paddingBottom:100}}>
        ...
        {/* ── أقسام القائمة (تحت السلايدر، ثابتة) ── */}
        {menuSections.length>0&&(
          <div className="sec-tabs" style={{padding:"14px 15px 0"}}>
          ...
          {grouped.filter(s=>s.items.length>0).map(sec=>(
            <div key={sec.id} ref={el=>sectionRefs.current[sec.id]=el} data-section-id={sec.id} style={{marginBottom:28}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
                {sec.items.map((p,pi)=>(
                  <div key={p.id||p._id} className={`${p.comingSoon?"":"card-tap"} item-anim`} style={{animationDelay:`${pi*0.07}s`}}
        ...
        <div style={{padding:"16px 15px"}}>
          ...
          {detailSize&&eTotal>0&&(
            <div style={{background:"#141414",border:"1px solid #C8A96A1a",borderRadius:11,padding:"10px 13px",marginBottom:14}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",fontSize:".72rem",color:"#8B6B4A",marginBottom:6}}><span>الإضافات</span><span>+ {eTotal.toLocaleString()} ل.س</span></div>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:".84rem",fontWeight:700,color:"#C8A96A",borderTop:"1px solid #1e1e1e",paddingTop:6}}><span>المجموع</span><span>{totalNow.toLocaleString()} ل.س</span></div>
            </div>
          )}
      ...
      <div style={{padding:"16px 15px"}}>
        {cart.length===0?(
          ...
          <>
              ...
              {cart.map(item=>(
                <div key={item.uid} style={{background:"#141414",border:"1px solid #1a1a1a",borderRadius:13,padding:"12px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:9}}>
                    ...
                    <div style={{textAlign:"left",marginRight:9,flexShrink:0}}><div style={{fontSize:".78rem",fontWeight:700,color:"#C8A96A"}}>{item.numericPrice.toLocaleString()} ل.س</div></div>
                  </div>
            ...
            {/* استلام من الفرع: خريطة الفرع */}
            {deliveryType==="pickup"&&(
              <div style={{marginBottom:18}}>
                <p style={{fontSize:".6rem",color:"#C8A96A66",letterSpacing:"2px",marginBottom:10}}>📍 موقع الفرع</p>
                <div style={{background:"#141414",border:"1px solid #C8A96A1a",borderRadius:13,padding:13}}>
                  <p style={{fontSize:".8rem",fontWeight:700,color:"#C8A96A",marginBottom:3}}>{BRANCH.name}</p>
                  <p style={{fontSize:".66rem",color:"#8B6B4A",marginBottom:12}}>حماة — السوريا</p>
                  <BranchMap/>
                </div>
              </div>
            )}
